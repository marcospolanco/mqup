...
This is **not** MiniLM inference. It is a deterministic stand-in that keeps POI and query vectors in the same space for demo and eval reproducibility without bundling a Core ML model in the 5-day window.

## Why not Core ML MiniLM in v1?

The spec's Day 2 deliverable called for Core ML conversion of `all-MiniLM-L6-v2`. That path is **staged, not skipped**:
...
**Interview answer:** "I shipped the retrieval architecture and eval loop first with a deterministic embedding oracle so BM25 vs hybrid and constraint behavior could be validated independently of model conversion risk. Core ML MiniLM is a drop-in replacement at the query embedding boundary; POI vectors can be regenerated from the same script with `--model minilm`."
...
- Apple `NLContextualEmbedding` (iOS 18+) — see spec §3.8
